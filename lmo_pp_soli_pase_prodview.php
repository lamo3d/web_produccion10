...
<?php include_once "lmo_pp_soli_pase_prodinfo.php" ?>
<?php include_once "lmo_pp_proydesinfo.php" ?>
<?php include_once "lmo_pp_usuariosinfo.php" ?>
<?php include_once "lmo_pp_revisiongridcls.php" ?>
...
$pp_soli_pase_prod_view = NULL; // Initialize page object first

class cpp_soli_pase_prod_view extends cpp_soli_pase_prod {
	...
	// Table name
	var $TableName = 'pp_soli_pase_prod';

	// Page object name
	var $PageObjName = 'pp_soli_pase_prod_view';
	...
	function __construct() {
		...
		// Table object (pp_soli_pase_prod)
		if (!isset($GLOBALS["pp_soli_pase_prod"]) || get_class($GLOBALS["pp_soli_pase_prod"]) == "cpp_soli_pase_prod") {
			$GLOBALS["pp_soli_pase_prod"] = &$this;
			$GLOBALS["Table"] = &$GLOBALS["pp_soli_pase_prod"];
		}
		$KeyUrl = "";
		if (@$_GET["IdPase"] <> "") {
		...
		// Table object (pp_proydes)
		if (!isset($GLOBALS['pp_proydes'])) $GLOBALS['pp_proydes'] = new cpp_proydes();
		...
		if (!defined("EW_TABLE_NAME"))
			define("EW_TABLE_NAME", 'pp_soli_pase_prod', TRUE);
	...
	function Page_Init() {
		...
		if (!$Security->CanView()) {
			...
			$this->Page_Terminate("lmo_pp_soli_pase_prodlist.php");
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					...
					if ($this->TotalRecs <= 0) { // No record found
						...
						$this->Page_Terminate("lmo_pp_soli_pase_prodlist.php"); // Return to list page
					...
					if (!$bMatchRecord) {
						if ($this->getSuccessMessage() == "" && $this->getFailureMessage() == "")
							$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record message
						$sReturnUrl = "lmo_pp_soli_pase_prodlist.php"; // No matching record, return to list
					} else {
			...
			$sReturnUrl = "lmo_pp_soli_pase_prodlist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupOtherOptions() {
		...
		// Detail table 'pp_revision'
		$body = $Language->TablePhrase("pp_revision", "TblCaption");
		$body .= str_replace("%c", $this->pp_revision_Count, $Language->Phrase("DetailCount"));
		$body = "<a class=\"ewAction ewDetailList\" href=\"" . ew_HtmlEncode("lmo_pp_revisionlist.php?" . EW_TABLE_SHOW_MASTER . "=pp_soli_pase_prod&IdPase=" . strval($this->IdPase->CurrentValue) . "") . "\">" . $body . "</a>";
		$item = &$option->Add("detail_pp_revision");
		$item->Body = $body;
		$item->Visible = $Security->AllowList(CurrentProjectID() . 'pp_revision');
		if ($item->Visible) {
			if ($DetailTableLink <> "") $DetailTableLink .= ",";
			$DetailTableLink .= "pp_revision";
		}
	...
	function LoadRowValues(&$rs) {
		...
		if (!isset($GLOBALS["pp_revision_grid"])) $GLOBALS["pp_revision_grid"] = new cpp_revision_grid;
		$sDetailFilter = $GLOBALS["pp_revision"]->SqlDetailFilter_pp_soli_pase_prod();
		$sDetailFilter = str_replace("@IdPase@", ew_AdjustSql($this->IdPase->DbValue), $sDetailFilter);
		$GLOBALS["pp_revision"]->setCurrentMasterTable("pp_soli_pase_prod");
		$sDetailFilter = $GLOBALS["pp_revision"]->ApplyUserIDFilters($sDetailFilter);
		$this->pp_revision_Count = $GLOBALS["pp_revision"]->LoadRecordCount($sDetailFilter);
	}
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			$this->Titulo->LinkAttrs["data-tooltip-id"] = "tt_pp_soli_pase_prod_x_Titulo";
			...
			$this->id_qa->LinkAttrs["data-tooltip-id"] = "tt_pp_soli_pase_prod_x_id_qa";
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf_pp_soli_pase_prod\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_pp_soli_pase_prod',hdr:ewLanguage.Phrase('ExportToEmail'),f:document.fpp_soli_pase_prodview,key:" . ew_ArrayToJsonAttr($this->RecKey) . ",sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("pp_revision", $DetailTblVar)) {
				if (!isset($GLOBALS["pp_revision_grid"]))
					$GLOBALS["pp_revision_grid"] = new cpp_revision_grid;
				if ($GLOBALS["pp_revision_grid"]->DetailView) {
					$GLOBALS["pp_revision_grid"]->CurrentMode = "view";

					// Save current master table to detail table
					$GLOBALS["pp_revision_grid"]->setCurrentMasterTable($this->TableVar);
					$GLOBALS["pp_revision_grid"]->setStartRecordNumber(1);
					$GLOBALS["pp_revision_grid"]->IdPase->FldIsDetailKey = TRUE;
					$GLOBALS["pp_revision_grid"]->IdPase->CurrentValue = $this->IdPase->CurrentValue;
					$GLOBALS["pp_revision_grid"]->IdPase->setSessionValue($GLOBALS["pp_revision_grid"]->IdPase->CurrentValue);
				}
			}
		}
	}
	...
	function SetupBreadcrumb() {
		...
		$Breadcrumb->Add("list", $this->TableVar, "lmo_pp_soli_pase_prodlist.php", $this->TableVar, TRUE);
		$PageId = "view";
...
// Create page object
if (!isset($pp_soli_pase_prod_view)) $pp_soli_pase_prod_view = new cpp_soli_pase_prod_view();

// Page init
$pp_soli_pase_prod_view->Page_Init();

// Page main
$pp_soli_pase_prod_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$pp_soli_pase_prod_view->Page_Render();
?>
<?php include_once "lmo_header.php" ?>
<?php if ($pp_soli_pase_prod->Export == "") { ?>
<script type="text/javascript">

// Page object
var pp_soli_pase_prod_view = new ew_Page("pp_soli_pase_prod_view");
pp_soli_pase_prod_view.PageID = "view"; // Page ID
var EW_PAGE_ID = pp_soli_pase_prod_view.PageID; // For backward compatibility

// Form object
var fpp_soli_pase_prodview = new ew_Form("fpp_soli_pase_prodview");

// Form_CustomValidate event
fpp_soli_pase_prodview.Form_CustomValidate = 
...
fpp_soli_pase_prodview.ValidateRequired = true;
<?php } else { ?>
fpp_soli_pase_prodview.ValidateRequired = false; 
<?php } ?>

// Multi-Page properties
fpp_soli_pase_prodview.MultiPage = new ew_MultiPage("fpp_soli_pase_prodview",
	[["x_IdPase",1],["x_idProydes",1],["x_Fecha",1],["x_FechaPuesta",1],["x_Titulo",1],["x_idempresa",1],["x_CodServicio",1],["x_IdMotivo",1],["x_Prioridad",1],["x_TipoSolicitud",1],["x_CerrarRequerimiento",1],["x_empresa_costo",1],["x_horasdesarrollo",1],["x_CodUsuarioLider",1],["x_CodUsuario",1],["x_idproveedor",1],["x_Descripcion",1],["x_Instrucciones",1],["x_ArchAdjuntos",1],["x_querys",2],["x_Adjuntos",1],["x_stores",1],["x_ejecutables",1],["x_SolicitudDesarrollo",1],["x_PlanPruebas",1],["x_ManualUsuario",1],["x_DiagramaER",1],["x_DiagramaProcesos",1],["x_DiccionarioDatos",1],["x_IdEstadoSoliPuestaProd",1],["x_idanalista_ss",1],["x_idjefeproy_ss",1],["x_fechapase_ss",1],["x_horasdesarrollo_ss",1],["x_id_qa",1],["x_observaciones_qa",1],["x_fecha_asigna_qa",1]]
);

// Dynamic selection lists
fpp_soli_pase_prodview.Lists["x_idempresa"] = {"LinkField":"x_Idempresa","Ajax":null,"AutoFill":false,"DisplayFields":["x_empresa","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fpp_soli_pase_prodview.Lists["x_CodServicio"] = {"LinkField":"x_IdServicio","Ajax":true,"AutoFill":false,"DisplayFields":["x_Servicio","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fpp_soli_pase_prodview.Lists["x_IdMotivo"] = {"LinkField":"x_IdMotivo","Ajax":null,"AutoFill":false,"DisplayFields":["x_Motivo","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fpp_soli_pase_prodview.Lists["x_TipoSolicitud[]"] = {"LinkField":"x_TipoSolicitud","Ajax":null,"AutoFill":false,"DisplayFields":["x_TipoSolicitud","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fpp_soli_pase_prodview.Lists["x_empresa_costo[]"] = {"LinkField":"x_Idempresa","Ajax":null,"AutoFill":false,"DisplayFields":["x_empresa","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fpp_soli_pase_prodview.Lists["x_CodUsuarioLider"] = {"LinkField":"x_idUsuario","Ajax":null,"AutoFill":false,"DisplayFields":["x_Nombre","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fpp_soli_pase_prodview.Lists["x_CodUsuario"] = {"LinkField":"x_CodUsuario","Ajax":true,"AutoFill":false,"DisplayFields":["x__login","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fpp_soli_pase_prodview.Lists["x_idproveedor"] = {"LinkField":"x_Idempresa","Ajax":true,"AutoFill":false,"DisplayFields":["x_empresa","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fpp_soli_pase_prodview.Lists["x_IdEstadoSoliPuestaProd"] = {"LinkField":"x_IdEstadoSoliPuestaProd","Ajax":null,"AutoFill":false,"DisplayFields":["x_estado","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fpp_soli_pase_prodview.Lists["x_idanalista_ss"] = {"LinkField":"x_idUsuario","Ajax":true,"AutoFill":false,"DisplayFields":["x__login","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fpp_soli_pase_prodview.Lists["x_idjefeproy_ss"] = {"LinkField":"x_idUsuario","Ajax":true,"AutoFill":false,"DisplayFields":["x__login","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fpp_soli_pase_prodview.Lists["x_id_qa"] = {"LinkField":"x_idUsuario","Ajax":true,"AutoFill":false,"DisplayFields":["x__login","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $pp_soli_pase_prod_view->ExportOptions->Render("body") ?>
<?php if (!$pp_soli_pase_prod_view->ExportOptions->UseDropDownButton) { ?>
</div>
<div class="ewViewOtherOptions">
<?php } ?>
<?php
	foreach ($pp_soli_pase_prod_view->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<?php } ?>
<?php $pp_soli_pase_prod_view->ShowPageHeader(); ?>
<?php
$pp_soli_pase_prod_view->ShowMessage();
?>
<?php if ($pp_soli_pase_prod->Export == "") { ?>
<form name="ewPagerForm" class="ewForm form-inline" action="<?php echo ew_CurrentPage() ?>">
<table class="ewPager">
<tr><td>
<?php if (!isset($pp_soli_pase_prod_view->Pager)) $pp_soli_pase_prod_view->Pager = new cPrevNextPager($pp_soli_pase_prod_view->StartRec, $pp_soli_pase_prod_view->DisplayRecs, $pp_soli_pase_prod_view->TotalRecs) ?>
<?php if ($pp_soli_pase_prod_view->Pager->RecordCount > 0) { ?>
<table class="ewStdTable"><tbody><tr><td>
	<?php echo $Language->Phrase("Page") ?>&nbsp;
<div class="input-prepend input-append">
<!--first page button-->
	<?php if ($pp_soli_pase_prod_view->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-small" href="<?php echo $pp_soli_pase_prod_view->PageUrl() ?>start=<?php echo $pp_soli_pase_prod_view->Pager->FirstButton->Start ?>"><i class="icon-step-backward"></i></a>
	<?php } else { ?>
	<a class="btn btn-small disabled"><i class="icon-step-backward"></i></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($pp_soli_pase_prod_view->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-small" href="<?php echo $pp_soli_pase_prod_view->PageUrl() ?>start=<?php echo $pp_soli_pase_prod_view->Pager->PrevButton->Start ?>"><i class="icon-prev"></i></a>
	<?php } else { ?>
	<a class="btn btn-small disabled"><i class="icon-prev"></i></a>
	<?php } ?>
<!--current page number-->
	<input class="input-mini" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $pp_soli_pase_prod_view->Pager->CurrentPage ?>">
<!--next page button-->
	<?php if ($pp_soli_pase_prod_view->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-small" href="<?php echo $pp_soli_pase_prod_view->PageUrl() ?>start=<?php echo $pp_soli_pase_prod_view->Pager->NextButton->Start ?>"><i class="icon-play"></i></a>
	<?php } else { ?>
	<a class="btn btn-small disabled"><i class="icon-play"></i></a>
	<?php } ?>
<!--last page button-->
	<?php if ($pp_soli_pase_prod_view->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-small" href="<?php echo $pp_soli_pase_prod_view->PageUrl() ?>start=<?php echo $pp_soli_pase_prod_view->Pager->LastButton->Start ?>"><i class="icon-step-forward"></i></a>
	<?php } else { ?>
	<a class="btn btn-small disabled"><i class="icon-step-forward"></i></a>
	<?php } ?>
</div>
	&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $pp_soli_pase_prod_view->Pager->PageCount ?>
</td>
</tr></tbody></table>
<?php } else { ?>
...
<?php } ?>
<form name="fpp_soli_pase_prodview" id="fpp_soli_pase_prodview" class="ewForm form-inline" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="pp_soli_pase_prod">
<?php if ($pp_soli_pase_prod->Export == "") { ?>
<table class="ewStdTable"><tbody><tr><td>
<div class="tabbable" id="pp_soli_pase_prod_view">
	<ul class="nav nav-tabs">
		<li class="active"><a href="#tab_pp_soli_pase_prod1" data-toggle="tab"><?php echo $pp_soli_pase_prod->PageCaption(1) ?></a></li>
		<li><a href="#tab_pp_soli_pase_prod2" data-toggle="tab"><?php echo $pp_soli_pase_prod->PageCaption(2) ?></a></li>
	</ul>
	<div class="tab-content">
<?php } ?>
<?php if ($pp_soli_pase_prod->Export == "") { ?>
		<div class="tab-pane active" id="tab_pp_soli_pase_prod1">
<?php } ?>
<table class="ewGrid"<?php if ($pp_soli_pase_prod->Export == "") echo " style=\"width: 100%\""; ?>><tr><td>
<table id="tbl_pp_soli_pase_prodview1" class="table table-bordered table-striped">
<?php if ($pp_soli_pase_prod->IdPase->Visible) { // IdPase ?>
	<tr id="r_IdPase">
		<td><span id="elh_pp_soli_pase_prod_IdPase"><?php echo $pp_soli_pase_prod->IdPase->FldCaption() ?></span></td>
		<td<?php echo $pp_soli_pase_prod->IdPase->CellAttributes() ?>>
<span id="el_pp_soli_pase_prod_IdPase" class="control-group">
<span<?php echo $pp_soli_pase_prod->IdPase->ViewAttributes() ?>>
<?php echo $pp_soli_pase_prod->IdPase->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($pp_soli_pase_prod->idProydes->Visible) { // idProydes ?>
	<tr id="r_idProydes">
		<td><span id="elh_pp_soli_pase_prod_idProydes"><?php echo $pp_soli_pase_prod->idProydes->FldCaption() ?></span></td>
		<td<?php echo $pp_soli_pase_prod->idProydes->CellAttributes() ?>>
<span id="el_pp_soli_pase_prod_idProydes" class="control-group">
<span<?php echo $pp_soli_pase_prod->idProydes->ViewAttributes() ?>>
<?php echo $pp_soli_pase_prod->idProydes->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($pp_soli_pase_prod->Fecha->Visible) { // Fecha ?>
	<tr id="r_Fecha">
		<td><span id="elh_pp_soli_pase_prod_Fecha"><?php echo $pp_soli_pase_prod->Fecha->FldCaption() ?></span></td>
		<td<?php echo $pp_soli_pase_prod->Fecha->CellAttributes() ?>>
<span id="el_pp_soli_pase_prod_Fecha" class="control-group">
<span<?php echo $pp_soli_pase_prod->Fecha->ViewAttributes() ?>>
<?php echo $pp_soli_pase_prod->Fecha->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($pp_soli_pase_prod->FechaPuesta->Visible) { // FechaPuesta ?>
	<tr id="r_FechaPuesta">
		<td><span id="elh_pp_soli_pase_prod_FechaPuesta"><?php echo $pp_soli_pase_prod->FechaPuesta->FldCaption() ?></span></td>
		<td<?php echo $pp_soli_pase_prod->FechaPuesta->CellAttributes() ?>>
<span id="el_pp_soli_pase_prod_FechaPuesta" class="control-group">
<span<?php echo $pp_soli_pase_prod->FechaPuesta->ViewAttributes() ?>>
<?php echo $pp_soli_pase_prod->FechaPuesta->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($pp_soli_pase_prod->Titulo->Visible) { // Titulo ?>
	<tr id="r_Titulo">
		<td><span id="elh_pp_soli_pase_prod_Titulo"><?php echo $pp_soli_pase_prod->Titulo->FldCaption() ?></span></td>
		<td<?php echo $pp_soli_pase_prod->Titulo->CellAttributes() ?>>
<span id="el_pp_soli_pase_prod_Titulo" class="control-group">
<span<?php echo $pp_soli_pase_prod->Titulo->ViewAttributes() ?>>
<?php if (!ew_EmptyStr($pp_soli_pase_prod->Titulo->ViewValue) && $pp_soli_pase_prod->Titulo->LinkAttributes() <> "") { ?>
<a<?php echo $pp_soli_pase_prod->Titulo->LinkAttributes() ?>><?php echo $pp_soli_pase_prod->Titulo->ViewValue ?></a>
<?php } else { ?>
<?php echo $pp_soli_pase_prod->Titulo->ViewValue ?>
<?php } ?>
<span id="tt_pp_soli_pase_prod_x_Titulo" style="display: none">
<?php echo $pp_soli_pase_prod->Titulo->TooltipValue ?>
</span></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($pp_soli_pase_prod->idempresa->Visible) { // idempresa ?>
	<tr id="r_idempresa">
		<td><span id="elh_pp_soli_pase_prod_idempresa"><?php echo $pp_soli_pase_prod->idempresa->FldCaption() ?></span></td>
		<td<?php echo $pp_soli_pase_prod->idempresa->CellAttributes() ?>>
<span id="el_pp_soli_pase_prod_idempresa" class="control-group">
<span<?php echo $pp_soli_pase_prod->idempresa->ViewAttributes() ?>>
<?php echo $pp_soli_pase_prod->idempresa->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($pp_soli_pase_prod->CodServicio->Visible) { // CodServicio ?>
	<tr id="r_CodServicio">
		<td><span id="elh_pp_soli_pase_prod_CodServicio"><?php echo $pp_soli_pase_prod->CodServicio->FldCaption() ?></span></td>
		<td<?php echo $pp_soli_pase_prod->CodServicio->CellAttributes() ?>>
<span id="el_pp_soli_pase_prod_CodServicio" class="control-group">
<span<?php echo $pp_soli_pase_prod->CodServicio->ViewAttributes() ?>>
<?php echo $pp_soli_pase_prod->CodServicio->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($pp_soli_pase_prod->IdMotivo->Visible) { // IdMotivo ?>
	<tr id="r_IdMotivo">
		<td><span id="elh_pp_soli_pase_prod_IdMotivo"><?php echo $pp_soli_pase_prod->IdMotivo->FldCaption() ?></span></td>
		<td<?php echo $pp_soli_pase_prod->IdMotivo->CellAttributes() ?>>
<span id="el_pp_soli_pase_prod_IdMotivo" class="control-group">
<span<?php echo $pp_soli_pase_prod->IdMotivo->ViewAttributes() ?>>
<?php echo $pp_soli_pase_prod->IdMotivo->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($pp_soli_pase_prod->Prioridad->Visible) { // Prioridad ?>
	<tr id="r_Prioridad">
		<td><span id="elh_pp_soli_pase_prod_Prioridad"><?php echo $pp_soli_pase_prod->Prioridad->FldCaption() ?></span></td>
		<td<?php echo $pp_soli_pase_prod->Prioridad->CellAttributes() ?>>
<span id="el_pp_soli_pase_prod_Prioridad" class="control-group">
<span<?php echo $pp_soli_pase_prod->Prioridad->ViewAttributes() ?>>
<?php echo $pp_soli_pase_prod->Prioridad->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($pp_soli_pase_prod->TipoSolicitud->Visible) { // TipoSolicitud ?>
	<tr id="r_TipoSolicitud">
		<td><span id="elh_pp_soli_pase_prod_TipoSolicitud"><?php echo $pp_soli_pase_prod->TipoSolicitud->FldCaption() ?></span></td>
		<td<?php echo $pp_soli_pase_prod->TipoSolicitud->CellAttributes() ?>>
<span id="el_pp_soli_pase_prod_TipoSolicitud" class="control-group">
<span<?php echo $pp_soli_pase_prod->TipoSolicitud->ViewAttributes() ?>>
<?php echo $pp_soli_pase_prod->TipoSolicitud->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($pp_soli_pase_prod->CerrarRequerimiento->Visible) { // CerrarRequerimiento ?>
	<tr id="r_CerrarRequerimiento">
		<td><span id="elh_pp_soli_pase_prod_CerrarRequerimiento"><?php echo $pp_soli_pase_prod->CerrarRequerimiento->FldCaption() ?></span></td>
		<td<?php echo $pp_soli_pase_prod->CerrarRequerimiento->CellAttributes() ?>>
<span id="el_pp_soli_pase_prod_CerrarRequerimiento" class="control-group">
<span<?php echo $pp_soli_pase_prod->CerrarRequerimiento->ViewAttributes() ?>>
<?php echo $pp_soli_pase_prod->CerrarRequerimiento->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($pp_soli_pase_prod->empresa_costo->Visible) { // empresa_costo ?>
	<tr id="r_empresa_costo">
		<td><span id="elh_pp_soli_pase_prod_empresa_costo"><?php echo $pp_soli_pase_prod->empresa_costo->FldCaption() ?></span></td>
		<td<?php echo $pp_soli_pase_prod->empresa_costo->CellAttributes() ?>>
<span id="el_pp_soli_pase_prod_empresa_costo" class="control-group">
<span<?php echo $pp_soli_pase_prod->empresa_costo->ViewAttributes() ?>>
<?php echo $pp_soli_pase_prod->empresa_costo->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($pp_soli_pase_prod->horasdesarrollo->Visible) { // horasdesarrollo ?>
	<tr id="r_horasdesarrollo">
		<td><span id="elh_pp_soli_pase_prod_horasdesarrollo"><?php echo $pp_soli_pase_prod->horasdesarrollo->FldCaption() ?></span></td>
		<td<?php echo $pp_soli_pase_prod->horasdesarrollo->CellAttributes() ?>>
<span id="el_pp_soli_pase_prod_horasdesarrollo" class="control-group">
<span<?php echo $pp_soli_pase_prod->horasdesarrollo->ViewAttributes() ?>>
<?php echo $pp_soli_pase_prod->horasdesarrollo->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($pp_soli_pase_prod->CodUsuarioLider->Visible) { // CodUsuarioLider ?>
	<tr id="r_CodUsuarioLider">
		<td><span id="elh_pp_soli_pase_prod_CodUsuarioLider"><?php echo $pp_soli_pase_prod->CodUsuarioLider->FldCaption() ?></span></td>
		<td<?php echo $pp_soli_pase_prod->CodUsuarioLider->CellAttributes() ?>>
<span id="el_pp_soli_pase_prod_CodUsuarioLider" class="control-group">
<span<?php echo $pp_soli_pase_prod->CodUsuarioLider->ViewAttributes() ?>>
<?php echo $pp_soli_pase_prod->CodUsuarioLider->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($pp_soli_pase_prod->CodUsuario->Visible) { // CodUsuario ?>
	<tr id="r_CodUsuario">
		<td><span id="elh_pp_soli_pase_prod_CodUsuario"><?php echo $pp_soli_pase_prod->CodUsuario->FldCaption() ?></span></td>
		<td<?php echo $pp_soli_pase_prod->CodUsuario->CellAttributes() ?>>
<span id="el_pp_soli_pase_prod_CodUsuario" class="control-group">
<span<?php echo $pp_soli_pase_prod->CodUsuario->ViewAttributes() ?>>
<?php echo $pp_soli_pase_prod->CodUsuario->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($pp_soli_pase_prod->idproveedor->Visible) { // idproveedor ?>
	<tr id="r_idproveedor">
		<td><span id="elh_pp_soli_pase_prod_idproveedor"><?php echo $pp_soli_pase_prod->idproveedor->FldCaption() ?></span></td>
		<td<?php echo $pp_soli_pase_prod->idproveedor->CellAttributes() ?>>
<span id="el_pp_soli_pase_prod_idproveedor" class="control-group">
<span<?php echo $pp_soli_pase_prod->idproveedor->ViewAttributes() ?>>
<?php echo $pp_soli_pase_prod->idproveedor->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($pp_soli_pase_prod->Descripcion->Visible) { // Descripcion ?>
	<tr id="r_Descripcion">
		<td><span id="elh_pp_soli_pase_prod_Descripcion"><?php echo $pp_soli_pase_prod->Descripcion->FldCaption() ?></span></td>
		<td<?php echo $pp_soli_pase_prod->Descripcion->CellAttributes() ?>>
<span id="el_pp_soli_pase_prod_Descripcion" class="control-group">
<span<?php echo $pp_soli_pase_prod->Descripcion->ViewAttributes() ?>>
<?php echo $pp_soli_pase_prod->Descripcion->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($pp_soli_pase_prod->Instrucciones->Visible) { // Instrucciones ?>
	<tr id="r_Instrucciones">
		<td><span id="elh_pp_soli_pase_prod_Instrucciones"><?php echo $pp_soli_pase_prod->Instrucciones->FldCaption() ?></span></td>
		<td<?php echo $pp_soli_pase_prod->Instrucciones->CellAttributes() ?>>
<span id="el_pp_soli_pase_prod_Instrucciones" class="control-group">
<span<?php echo $pp_soli_pase_prod->Instrucciones->ViewAttributes() ?>>
<?php echo $pp_soli_pase_prod->Instrucciones->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($pp_soli_pase_prod->ArchAdjuntos->Visible) { // ArchAdjuntos ?>
	<tr id="r_ArchAdjuntos">
		<td><span id="elh_pp_soli_pase_prod_ArchAdjuntos"><?php echo $pp_soli_pase_prod->ArchAdjuntos->FldCaption() ?></span></td>
		<td<?php echo $pp_soli_pase_prod->ArchAdjuntos->CellAttributes() ?>>
<span id="el_pp_soli_pase_prod_ArchAdjuntos" class="control-group">
<span<?php echo $pp_soli_pase_prod->ArchAdjuntos->ViewAttributes() ?>>
<?php echo $pp_soli_pase_prod->ArchAdjuntos->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($pp_soli_pase_prod->Adjuntos->Visible) { // Adjuntos ?>
	<tr id="r_Adjuntos">
		<td><span id="elh_pp_soli_pase_prod_Adjuntos"><?php echo $pp_soli_pase_prod->Adjuntos->FldCaption() ?></span></td>
		<td<?php echo $pp_soli_pase_prod->Adjuntos->CellAttributes() ?>>
<span id="el_pp_soli_pase_prod_Adjuntos" class="control-group">
<span<?php echo $pp_soli_pase_prod->Adjuntos->ViewAttributes() ?>>
<?php if ($pp_soli_pase_prod->Adjuntos->LinkAttributes() <> "") { ?>
<?php if (!empty($pp_soli_pase_prod->Adjuntos->Upload->DbValue)) { ?>
<a<?php echo $pp_soli_pase_prod->Adjuntos->LinkAttributes() ?>><?php echo $pp_soli_pase_prod->Adjuntos->ViewValue ?></a>
<?php } elseif (!in_array($pp_soli_pase_prod->CurrentAction, array("I", "edit", "gridedit"))) { ?>	
&nbsp;
<?php } ?>
<?php } else { ?>
<?php if (!empty($pp_soli_pase_prod->Adjuntos->Upload->DbValue)) { ?>
<?php echo $pp_soli_pase_prod->Adjuntos->ViewValue ?>
<?php } elseif (!in_array($pp_soli_pase_prod->CurrentAction, array("I", "edit", "gridedit"))) { ?>	
&nbsp;
<?php } ?>
<?php } ?>
...
<?php } ?>
<?php if ($pp_soli_pase_prod->stores->Visible) { // stores ?>
	<tr id="r_stores">
		<td><span id="elh_pp_soli_pase_prod_stores"><?php echo $pp_soli_pase_prod->stores->FldCaption() ?></span></td>
		<td<?php echo $pp_soli_pase_prod->stores->CellAttributes() ?>>
<span id="el_pp_soli_pase_prod_stores" class="control-group">
<span<?php echo $pp_soli_pase_prod->stores->ViewAttributes() ?>>
<?php if ($pp_soli_pase_prod->stores->LinkAttributes() <> "") { ?>
<?php if (!empty($pp_soli_pase_prod->stores->Upload->DbValue)) { ?>
<a<?php echo $pp_soli_pase_prod->stores->LinkAttributes() ?>><?php echo $pp_soli_pase_prod->stores->ViewValue ?></a>
<?php } elseif (!in_array($pp_soli_pase_prod->CurrentAction, array("I", "edit", "gridedit"))) { ?>	
&nbsp;
<?php } ?>
<?php } else { ?>
<?php if (!empty($pp_soli_pase_prod->stores->Upload->DbValue)) { ?>
<?php echo $pp_soli_pase_prod->stores->ViewValue ?>
<?php } elseif (!in_array($pp_soli_pase_prod->CurrentAction, array("I", "edit", "gridedit"))) { ?>	
&nbsp;
<?php } ?>
<?php } ?>
...
<?php } ?>
<?php if ($pp_soli_pase_prod->ejecutables->Visible) { // ejecutables ?>
	<tr id="r_ejecutables">
		<td><span id="elh_pp_soli_pase_prod_ejecutables"><?php echo $pp_soli_pase_prod->ejecutables->FldCaption() ?></span></td>
		<td<?php echo $pp_soli_pase_prod->ejecutables->CellAttributes() ?>>
<span id="el_pp_soli_pase_prod_ejecutables" class="control-group">
<span<?php echo $pp_soli_pase_prod->ejecutables->ViewAttributes() ?>>
<?php if ($pp_soli_pase_prod->ejecutables->LinkAttributes() <> "") { ?>
<?php if (!empty($pp_soli_pase_prod->ejecutables->Upload->DbValue)) { ?>
<a<?php echo $pp_soli_pase_prod->ejecutables->LinkAttributes() ?>><?php echo $pp_soli_pase_prod->ejecutables->ViewValue ?></a>
<?php } elseif (!in_array($pp_soli_pase_prod->CurrentAction, array("I", "edit", "gridedit"))) { ?>	
&nbsp;
<?php } ?>
<?php } else { ?>
<?php if (!empty($pp_soli_pase_prod->ejecutables->Upload->DbValue)) { ?>
<?php echo $pp_soli_pase_prod->ejecutables->ViewValue ?>
<?php } elseif (!in_array($pp_soli_pase_prod->CurrentAction, array("I", "edit", "gridedit"))) { ?>	
&nbsp;
<?php } ?>
<?php } ?>
...
<?php } ?>
<?php if ($pp_soli_pase_prod->SolicitudDesarrollo->Visible) { // SolicitudDesarrollo ?>
	<tr id="r_SolicitudDesarrollo">
		<td><span id="elh_pp_soli_pase_prod_SolicitudDesarrollo"><?php echo $pp_soli_pase_prod->SolicitudDesarrollo->FldCaption() ?></span></td>
		<td<?php echo $pp_soli_pase_prod->SolicitudDesarrollo->CellAttributes() ?>>
<span id="el_pp_soli_pase_prod_SolicitudDesarrollo" class="control-group">
<span<?php echo $pp_soli_pase_prod->SolicitudDesarrollo->ViewAttributes() ?>>
<?php if ($pp_soli_pase_prod->SolicitudDesarrollo->LinkAttributes() <> "") { ?>
<?php if (!empty($pp_soli_pase_prod->SolicitudDesarrollo->Upload->DbValue)) { ?>
<a<?php echo $pp_soli_pase_prod->SolicitudDesarrollo->LinkAttributes() ?>><?php echo $pp_soli_pase_prod->SolicitudDesarrollo->ViewValue ?></a>
<?php } elseif (!in_array($pp_soli_pase_prod->CurrentAction, array("I", "edit", "gridedit"))) { ?>	
&nbsp;
<?php } ?>
<?php } else { ?>
<?php if (!empty($pp_soli_pase_prod->SolicitudDesarrollo->Upload->DbValue)) { ?>
<?php echo $pp_soli_pase_prod->SolicitudDesarrollo->ViewValue ?>
<?php } elseif (!in_array($pp_soli_pase_prod->CurrentAction, array("I", "edit", "gridedit"))) { ?>	
&nbsp;
<?php } ?>
<?php } ?>
...
<?php } ?>
<?php if ($pp_soli_pase_prod->PlanPruebas->Visible) { // PlanPruebas ?>
	<tr id="r_PlanPruebas">
		<td><span id="elh_pp_soli_pase_prod_PlanPruebas"><?php echo $pp_soli_pase_prod->PlanPruebas->FldCaption() ?></span></td>
		<td<?php echo $pp_soli_pase_prod->PlanPruebas->CellAttributes() ?>>
<span id="el_pp_soli_pase_prod_PlanPruebas" class="control-group">
<span<?php echo $pp_soli_pase_prod->PlanPruebas->ViewAttributes() ?>>
<?php if ($pp_soli_pase_prod->PlanPruebas->LinkAttributes() <> "") { ?>
<?php if (!empty($pp_soli_pase_prod->PlanPruebas->Upload->DbValue)) { ?>
<a<?php echo $pp_soli_pase_prod->PlanPruebas->LinkAttributes() ?>><?php echo $pp_soli_pase_prod->PlanPruebas->ViewValue ?></a>
<?php } elseif (!in_array($pp_soli_pase_prod->CurrentAction, array("I", "edit", "gridedit"))) { ?>	
&nbsp;
<?php } ?>
<?php } else { ?>
<?php if (!empty($pp_soli_pase_prod->PlanPruebas->Upload->DbValue)) { ?>
<?php echo $pp_soli_pase_prod->PlanPruebas->ViewValue ?>
<?php } elseif (!in_array($pp_soli_pase_prod->CurrentAction, array("I", "edit", "gridedit"))) { ?>	
&nbsp;
<?php } ?>
<?php } ?>
...
<?php } ?>
<?php if ($pp_soli_pase_prod->ManualUsuario->Visible) { // ManualUsuario ?>
	<tr id="r_ManualUsuario">
		<td><span id="elh_pp_soli_pase_prod_ManualUsuario"><?php echo $pp_soli_pase_prod->ManualUsuario->FldCaption() ?></span></td>
		<td<?php echo $pp_soli_pase_prod->ManualUsuario->CellAttributes() ?>>
<span id="el_pp_soli_pase_prod_ManualUsuario" class="control-group">
<span<?php echo $pp_soli_pase_prod->ManualUsuario->ViewAttributes() ?>>
<?php if ($pp_soli_pase_prod->ManualUsuario->LinkAttributes() <> "") { ?>
<?php if (!empty($pp_soli_pase_prod->ManualUsuario->Upload->DbValue)) { ?>
<a<?php echo $pp_soli_pase_prod->ManualUsuario->LinkAttributes() ?>><?php echo $pp_soli_pase_prod->ManualUsuario->ViewValue ?></a>
<?php } elseif (!in_array($pp_soli_pase_prod->CurrentAction, array("I", "edit", "gridedit"))) { ?>	
&nbsp;
<?php } ?>
<?php } else { ?>
<?php if (!empty($pp_soli_pase_prod->ManualUsuario->Upload->DbValue)) { ?>
<?php echo $pp_soli_pase_prod->ManualUsuario->ViewValue ?>
<?php } elseif (!in_array($pp_soli_pase_prod->CurrentAction, array("I", "edit", "gridedit"))) { ?>	
&nbsp;
<?php } ?>
<?php } ?>
...
<?php } ?>
<?php if ($pp_soli_pase_prod->DiagramaER->Visible) { // DiagramaER ?>
	<tr id="r_DiagramaER">
		<td><span id="elh_pp_soli_pase_prod_DiagramaER"><?php echo $pp_soli_pase_prod->DiagramaER->FldCaption() ?></span></td>
		<td<?php echo $pp_soli_pase_prod->DiagramaER->CellAttributes() ?>>
<span id="el_pp_soli_pase_prod_DiagramaER" class="control-group">
<span<?php echo $pp_soli_pase_prod->DiagramaER->ViewAttributes() ?>>
<?php if ($pp_soli_pase_prod->DiagramaER->LinkAttributes() <> "") { ?>
<?php if (!empty($pp_soli_pase_prod->DiagramaER->Upload->DbValue)) { ?>
<a<?php echo $pp_soli_pase_prod->DiagramaER->LinkAttributes() ?>><?php echo $pp_soli_pase_prod->DiagramaER->ViewValue ?></a>
<?php } elseif (!in_array($pp_soli_pase_prod->CurrentAction, array("I", "edit", "gridedit"))) { ?>	
&nbsp;
<?php } ?>
<?php } else { ?>
<?php if (!empty($pp_soli_pase_prod->DiagramaER->Upload->DbValue)) { ?>
<?php echo $pp_soli_pase_prod->DiagramaER->ViewValue ?>
<?php } elseif (!in_array($pp_soli_pase_prod->CurrentAction, array("I", "edit", "gridedit"))) { ?>	
&nbsp;
<?php } ?>
<?php } ?>
...
<?php } ?>
<?php if ($pp_soli_pase_prod->DiagramaProcesos->Visible) { // DiagramaProcesos ?>
	<tr id="r_DiagramaProcesos">
		<td><span id="elh_pp_soli_pase_prod_DiagramaProcesos"><?php echo $pp_soli_pase_prod->DiagramaProcesos->FldCaption() ?></span></td>
		<td<?php echo $pp_soli_pase_prod->DiagramaProcesos->CellAttributes() ?>>
<span id="el_pp_soli_pase_prod_DiagramaProcesos" class="control-group">
<span<?php echo $pp_soli_pase_prod->DiagramaProcesos->ViewAttributes() ?>>
<?php if ($pp_soli_pase_prod->DiagramaProcesos->LinkAttributes() <> "") { ?>
<?php if (!empty($pp_soli_pase_prod->DiagramaProcesos->Upload->DbValue)) { ?>
<a<?php echo $pp_soli_pase_prod->DiagramaProcesos->LinkAttributes() ?>><?php echo $pp_soli_pase_prod->DiagramaProcesos->ViewValue ?></a>
<?php } elseif (!in_array($pp_soli_pase_prod->CurrentAction, array("I", "edit", "gridedit"))) { ?>	
&nbsp;
<?php } ?>
<?php } else { ?>
<?php if (!empty($pp_soli_pase_prod->DiagramaProcesos->Upload->DbValue)) { ?>
<?php echo $pp_soli_pase_prod->DiagramaProcesos->ViewValue ?>
<?php } elseif (!in_array($pp_soli_pase_prod->CurrentAction, array("I", "edit", "gridedit"))) { ?>	
&nbsp;
<?php } ?>
<?php } ?>
...
<?php } ?>
<?php if ($pp_soli_pase_prod->DiccionarioDatos->Visible) { // DiccionarioDatos ?>
	<tr id="r_DiccionarioDatos">
		<td><span id="elh_pp_soli_pase_prod_DiccionarioDatos"><?php echo $pp_soli_pase_prod->DiccionarioDatos->FldCaption() ?></span></td>
		<td<?php echo $pp_soli_pase_prod->DiccionarioDatos->CellAttributes() ?>>
<span id="el_pp_soli_pase_prod_DiccionarioDatos" class="control-group">
<span<?php echo $pp_soli_pase_prod->DiccionarioDatos->ViewAttributes() ?>>
<?php if ($pp_soli_pase_prod->DiccionarioDatos->LinkAttributes() <> "") { ?>
<?php if (!empty($pp_soli_pase_prod->DiccionarioDatos->Upload->DbValue)) { ?>
<a<?php echo $pp_soli_pase_prod->DiccionarioDatos->LinkAttributes() ?>><?php echo $pp_soli_pase_prod->DiccionarioDatos->ViewValue ?></a>
<?php } elseif (!in_array($pp_soli_pase_prod->CurrentAction, array("I", "edit", "gridedit"))) { ?>	
&nbsp;
<?php } ?>
<?php } else { ?>
<?php if (!empty($pp_soli_pase_prod->DiccionarioDatos->Upload->DbValue)) { ?>
<?php echo $pp_soli_pase_prod->DiccionarioDatos->ViewValue ?>
<?php } elseif (!in_array($pp_soli_pase_prod->CurrentAction, array("I", "edit", "gridedit"))) { ?>	
&nbsp;
<?php } ?>
<?php } ?>
...
<?php } ?>
<?php if ($pp_soli_pase_prod->IdEstadoSoliPuestaProd->Visible) { // IdEstadoSoliPuestaProd ?>
	<tr id="r_IdEstadoSoliPuestaProd">
		<td><span id="elh_pp_soli_pase_prod_IdEstadoSoliPuestaProd"><?php echo $pp_soli_pase_prod->IdEstadoSoliPuestaProd->FldCaption() ?></span></td>
		<td<?php echo $pp_soli_pase_prod->IdEstadoSoliPuestaProd->CellAttributes() ?>>
<span id="el_pp_soli_pase_prod_IdEstadoSoliPuestaProd" class="control-group">
<span<?php echo $pp_soli_pase_prod->IdEstadoSoliPuestaProd->ViewAttributes() ?>>
<?php echo $pp_soli_pase_prod->IdEstadoSoliPuestaProd->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($pp_soli_pase_prod->idanalista_ss->Visible) { // idanalista_ss ?>
	<tr id="r_idanalista_ss">
		<td><span id="elh_pp_soli_pase_prod_idanalista_ss"><?php echo $pp_soli_pase_prod->idanalista_ss->FldCaption() ?></span></td>
		<td<?php echo $pp_soli_pase_prod->idanalista_ss->CellAttributes() ?>>
<span id="el_pp_soli_pase_prod_idanalista_ss" class="control-group">
<span<?php echo $pp_soli_pase_prod->idanalista_ss->ViewAttributes() ?>>
<?php echo $pp_soli_pase_prod->idanalista_ss->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($pp_soli_pase_prod->idjefeproy_ss->Visible) { // idjefeproy_ss ?>
	<tr id="r_idjefeproy_ss">
		<td><span id="elh_pp_soli_pase_prod_idjefeproy_ss"><?php echo $pp_soli_pase_prod->idjefeproy_ss->FldCaption() ?></span></td>
		<td<?php echo $pp_soli_pase_prod->idjefeproy_ss->CellAttributes() ?>>
<span id="el_pp_soli_pase_prod_idjefeproy_ss" class="control-group">
<span<?php echo $pp_soli_pase_prod->idjefeproy_ss->ViewAttributes() ?>>
<?php echo $pp_soli_pase_prod->idjefeproy_ss->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($pp_soli_pase_prod->fechapase_ss->Visible) { // fechapase_ss ?>
	<tr id="r_fechapase_ss">
		<td><span id="elh_pp_soli_pase_prod_fechapase_ss"><?php echo $pp_soli_pase_prod->fechapase_ss->FldCaption() ?></span></td>
		<td<?php echo $pp_soli_pase_prod->fechapase_ss->CellAttributes() ?>>
<span id="el_pp_soli_pase_prod_fechapase_ss" class="control-group">
<span<?php echo $pp_soli_pase_prod->fechapase_ss->ViewAttributes() ?>>
<?php echo $pp_soli_pase_prod->fechapase_ss->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($pp_soli_pase_prod->horasdesarrollo_ss->Visible) { // horasdesarrollo_ss ?>
	<tr id="r_horasdesarrollo_ss">
		<td><span id="elh_pp_soli_pase_prod_horasdesarrollo_ss"><?php echo $pp_soli_pase_prod->horasdesarrollo_ss->FldCaption() ?></span></td>
		<td<?php echo $pp_soli_pase_prod->horasdesarrollo_ss->CellAttributes() ?>>
<span id="el_pp_soli_pase_prod_horasdesarrollo_ss" class="control-group">
<span<?php echo $pp_soli_pase_prod->horasdesarrollo_ss->ViewAttributes() ?>>
<?php echo $pp_soli_pase_prod->horasdesarrollo_ss->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($pp_soli_pase_prod->id_qa->Visible) { // id_qa ?>
	<tr id="r_id_qa">
		<td><span id="elh_pp_soli_pase_prod_id_qa"><?php echo $pp_soli_pase_prod->id_qa->FldCaption() ?></span></td>
		<td<?php echo $pp_soli_pase_prod->id_qa->CellAttributes() ?>>
<span id="el_pp_soli_pase_prod_id_qa" class="control-group">
<span<?php echo $pp_soli_pase_prod->id_qa->ViewAttributes() ?>>
<?php if (!ew_EmptyStr($pp_soli_pase_prod->id_qa->ViewValue) && $pp_soli_pase_prod->id_qa->LinkAttributes() <> "") { ?>
<a<?php echo $pp_soli_pase_prod->id_qa->LinkAttributes() ?>><?php echo $pp_soli_pase_prod->id_qa->ViewValue ?></a>
<?php } else { ?>
<?php echo $pp_soli_pase_prod->id_qa->ViewValue ?>
<?php } ?>
<span id="tt_pp_soli_pase_prod_x_id_qa" style="display: none">
<?php echo $pp_soli_pase_prod->id_qa->TooltipValue ?>
</span></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($pp_soli_pase_prod->observaciones_qa->Visible) { // observaciones_qa ?>
	<tr id="r_observaciones_qa">
		<td><span id="elh_pp_soli_pase_prod_observaciones_qa"><?php echo $pp_soli_pase_prod->observaciones_qa->FldCaption() ?></span></td>
		<td<?php echo $pp_soli_pase_prod->observaciones_qa->CellAttributes() ?>>
<span id="el_pp_soli_pase_prod_observaciones_qa" class="control-group">
<span<?php echo $pp_soli_pase_prod->observaciones_qa->ViewAttributes() ?>>
<?php echo $pp_soli_pase_prod->observaciones_qa->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($pp_soli_pase_prod->fecha_asigna_qa->Visible) { // fecha_asigna_qa ?>
	<tr id="r_fecha_asigna_qa">
		<td><span id="elh_pp_soli_pase_prod_fecha_asigna_qa"><?php echo $pp_soli_pase_prod->fecha_asigna_qa->FldCaption() ?></span></td>
		<td<?php echo $pp_soli_pase_prod->fecha_asigna_qa->CellAttributes() ?>>
<span id="el_pp_soli_pase_prod_fecha_asigna_qa" class="control-group">
<span<?php echo $pp_soli_pase_prod->fecha_asigna_qa->ViewAttributes() ?>>
<?php echo $pp_soli_pase_prod->fecha_asigna_qa->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
$strSql ="Select distinct pp_servicio.Servicio, pp_tablas.tabla, pp_tablas.diccionario, pp_tablas.Idtabla, pp_tablas.fecha From pp_tablas Inner Join pp_servicio On pp_servicio.IdServicio = pp_tablas.idbase  where pp_tablas.idpase = '". $pp_soli_pase_prod->IdPase->CurrentValue ."'";
$rslmo = $conn->Execute($strSql);
?>
...
$strSql ="SELECT pp_tablas.tabla, pp_servicio.Servicio, pp_gbcon.gbconpfij, pp_gbcon.gbconcorr, pp_gbcon.gbcondesc, pp_gbcon.gbconabre, pp_gbcon.id_pase FROM pp_gbcon INNER JOIN pp_tablas ON pp_tablas.Idtabla = pp_gbcon.id_tabla INNER JOIN pp_servicio ON pp_servicio.IdServicio = pp_gbcon.id_base WHERE   pp_gbcon.id_pase= '". $pp_soli_pase_prod->IdPase->CurrentValue ."' and pp_gbcon.idestado not in (3)";	
...
$strSql ="Select distinct pp_reqstoreproc.Idreqstore, pp_reqstoreproc.fechasepara, pp_reqstoreproc.idpase, pp_servicio.Servicio, pp_storeprocedure.idstoreproc, pp_storeprocedure.ddl, pp_storeprocedure.descripcion, pp_reqstoreproc.desmodifi, pp_reqstoreproc.deserror, pp_conceptos.descripcion As estado From pp_reqstoreproc Inner Join pp_storeprocedure On pp_storeprocedure.idstoreproc = pp_reqstoreproc.idstoreproc Inner Join pp_servicio On pp_servicio.IdServicio = pp_reqstoreproc.idSistema Inner Join pp_conceptos On pp_conceptos.iddetalle = pp_reqstoreproc.idestado Where pp_reqstoreproc.idpase = '". $pp_soli_pase_prod->IdPase->CurrentValue ."' And pp_reqstoreproc.pasarproduccion = 1 And pp_conceptos.idconcepto = 2 And pp_reqstoreproc.idestado In (1,2,3) order by 4,1"; 
$rslmo = $conn->Execute($strSql);
?>
...
<?php } ?>
<?php if ($pp_soli_pase_prod->Export == "") { ?>
		<div class="tab-pane" id="tab_pp_soli_pase_prod2">
<?php } ?>
<table class="ewGrid"<?php if ($pp_soli_pase_prod->Export == "") echo " style=\"width: 100%\""; ?>><tr><td>
<table id="tbl_pp_soli_pase_prodview2" class="table table-bordered table-striped">
<?php if ($pp_soli_pase_prod->querys->Visible) { // querys ?>
	<tr id="r_querys">
		<td><span id="elh_pp_soli_pase_prod_querys"><?php echo $pp_soli_pase_prod->querys->FldCaption() ?></span></td>
		<td<?php echo $pp_soli_pase_prod->querys->CellAttributes() ?>>
<span id="el_pp_soli_pase_prod_querys" class="control-group">
<span<?php echo $pp_soli_pase_prod->querys->ViewAttributes() ?>>
<?php echo $pp_soli_pase_prod->querys->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php } ?>
<?php
	if (in_array("pp_revision", explode(",", $pp_soli_pase_prod->getCurrentDetailTable())) && $pp_revision->DetailView) {
?>
<?php include_once "lmo_pp_revisiongrid.php" ?>
<?php } ?>
</form>
<script type="text/javascript">
fpp_soli_pase_prodview.Init();
</script>
<?php
$pp_soli_pase_prod_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($pp_soli_pase_prod->Export == "") { ?>
...
<?php
$pp_soli_pase_prod_view->Page_Terminate();
?>
